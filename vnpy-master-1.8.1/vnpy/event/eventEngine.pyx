...
cdef class EventEngine(object):
    ...
    #----------------------------------------------------------------------
    cpdef register(self, type_, handler):
        """注册事件处理函数监听"""
        # 尝试获取该事件类型对应的处理函数列表，若无cpdefaultDict会自动创建新的list
    ...
    #----------------------------------------------------------------------
    cpdef unregister(self, type_, handler):
        """注销事件处理函数监听"""
    ...
    #----------------------------------------------------------------------
    cpdef put(self, event):
        """向事件队列中存入事件"""
        self.__queue.put(event)
        
    #----------------------------------------------------------------------
    cpdef registerGeneralHandler(self, handler):
        """注册通用事件处理函数监听"""
        if handler not in self.__generalHandlers:
            self.__generalHandlers.append(handler)
            
    #----------------------------------------------------------------------
    cpdef unregisterGeneralHandler(self, handler):
        """注销通用事件处理函数监听"""
...
########################################################################
cdef class EventEngine2(object):
    """
