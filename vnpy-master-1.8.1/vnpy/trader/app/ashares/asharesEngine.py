...
########################################################################
class ASharesEngine(object):
    """CTA策略引擎"""
    settingFileName = 'AShares_setting.json'
    ...
    def __init__(self, mainEngine, eventEngine):
        ...
        ## =====================================================================        
        ## 需要订阅的合约
        # self.subscribeContracts = ['000002','300001','600010','600396']
        self.subscribeContracts = []
        for tb in ['tradingSignal']:
            try:
                tmp = self.fetchInstrumentID(self.accountID, tb)
        ...
        ## =====================================================================
        self.beginning = time(8, 00) # 日盘启动和停止时间
        self.ending = time(15, 15)
        self.exitCounter = 0
        ## =====================================================================
    ...
    def sendOrder(self, vtSymbol, orderType, price, volume, strategy):
        ...
        # 设计为 ASHARES 引擎发出的委托只允许使用限价单
        req.priceType = PRICETYPE_LIMITPRICE    
        
        # ASHARES 委托类型映射
        if orderType == ASHARES_BUY:
            req.direction = DIRECTION_LONG
        elif orderType == ASHARES_SELL:
        ...
        self.writeASharesLog(u'策略%s发送委托，%s，%s，%s@%s' 
    ...
    def processTickEvent(self, event):
        ...
        # 推送tick到对应的策略实例进行处理
        # if (tick.vtSymbol in self.tickStrategyDict or
        #     tick.symbol in self.tickStrategyDict):
        if tick.vtSymbol in self.tickStrategyDict:
            ...
            try:
                ...
                self.writeASharesLog(traceback.format_exc(), ERROR)
                return
                
            # 逐个推送到策略实例中
            l = self.tickStrategyDict[tick.vtSymbol]
            # l = self.tickStrategyDict[tick.symbol]
            for strategy in l:
    ...
    def processTradingStatus(self, event):
        ...
        ## ---------------------------------------------------------------------
        if not (self.beginning <= datetime.now().time() <= self.ending):
            self.exitCounter += 1
            self.writeASharesLog(u'即将退出系统，计数器：%s' %self.exitCounter)
    ...
    #----------------------------------------------------------------------
    def writeASharesLog(self, content, logLevel = INFO):
        """快速发出CTA模块日志事件"""
        log = VtLogData()
        log.logContent = content
        log.logLevel = logLevel
        log.gatewayName = 'ASHARES_STRATEGY'
    ...
    def loadStrategy(self, setting):
        ...
        try:
            ...
            self.writeASharesLog(u'载入策略出错：%s' %msg, ERROR)
            return
        
        # 获取策略类
        strategyClass = STRATEGY_CLASS.get(className, None)
        if not strategyClass:
            self.writeASharesLog(u'找不到策略类：%s' %className)
            return
        
        # 防止策略重名
        if name in self.strategyDict:
            self.writeASharesLog(u'策略实例重名：%s' %name)
        else:
            # 创建策略实例
    ...
    def subscribeMarketData(self, strategy):
        ...
        # 订阅合约
        # ----------------------------------------------------------------------
        for k in self.subscribeContracts:
            ## --------------------------------------
            if k in self.tickStrategyDict:
                l = self.tickStrategyDict[k]
            else:
                l = []
                self.tickStrategyDict[k] = l
            l.append(strategy)
            ## ---------------------------------------

            contract = self.mainEngine.getContract(k)
            if contract:
                req = VtSubscribeReq()
                req.symbol = contract.symbol
                req.exchange = contract.exchange
                ...
                self.writeASharesLog(u'%s的交易合约%s无法找到' %(strategy.name, strategy.vtSymbol))
        # ----------------------------------------------------------------------
    ...
    def initStrategy(self, name):
        ...
        if name in self.strategyDict:
            ...
            if not strategy.inited:
                ...
                self.writeASharesLog(u'请勿重复初始化策略实例：%s' %name)
        else:
            self.writeASharesLog(u'策略实例不存在：%s' %name)        
    ...
    def getStrategyVar(self, name):
        ...
        if name in self.strategyDict:
            ...
            self.writeASharesLog(u'策略实例不存在：' + name)    
            return None
    ...
    def callStrategyFunc(self, strategy, func, params=None):
        ...
        try:
            ...
            self.writeASharesLog(content, ERROR)
    ...
    def fetchInstrumentID(self, dbName, tbName):
        tmp = vtFunction.dbMySQLQuery(
            dbName,
            """
            select * from %s
            """ %(tbName))
        return(tmp.instrumentID.values)
