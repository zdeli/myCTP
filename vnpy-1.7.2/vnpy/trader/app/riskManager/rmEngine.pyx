...
########################################################################
cdef class RmEngine(object):
    """风控引擎"""

    cdef dict __dict__
    cdef public:
        int orderFlowCount, orderFlowLimit, orderFlowClear, orderFlowTimer
        int orderSizeLimit, tradeCount, tradeLimit, workingOrderLimit
        dict orderCancelDict
        dict marginRatioDict
        float marginRatioLimit
    ...
    def __cinit__(self, mainEngine, eventEngine):
        ...
        self.settingFileName = 'RM_setting.json'
        ...
        # 流控相关
        self.orderFlowCount = 0     # 单位时间内委托计数
        self.orderFlowLimit = 0     # 委托限制
        self.orderFlowClear = 0     # 计数清空时间（秒）
        self.orderFlowTimer = 0     # 计数清空时间计时

        # 单笔委托相关
        self.orderSizeLimit = 0     # 单笔委托最大限制

        # 成交统计相关
        self.tradeCount = 0         # 当日成交合约数量统计
        self.tradeLimit = 0         # 当日成交合约数量限制

        # 单品种撤单统计
        self.orderCancelLimit = 0   # 撤单总次数限制
        self.orderCancelDict = {}   # 单一合约对应撤单次数的字典

        # 活动合约相关
        self.workingOrderLimit = 0  # 活动合约最大限制
        
        # 保证金相关
        self.marginRatioDict = {}   # 保证金占账户净值比例字典
        self.marginRatioLimit = 0.0 # 最大比例限制
    ...
    #----------------------------------------------------------------------
    cpdef updateTrade(self, event):
        """更新成交数据"""
        trade = event.dict_['data']
        self.tradeCount += trade.volume

    #----------------------------------------------------------------------
    cpdef updateTimer(self, event):
        """更新定时器"""
    ...
    #----------------------------------------------------------------------
    cpdef updateAccount(self, event):
        """账户资金更新"""
    ...
    #----------------------------------------------------------------------
    cpdef writeRiskLog(self, content):
        """快速发出日志事件"""
        # 发出报警提示音
    ...
    #----------------------------------------------------------------------
    cpdef checkRisk(self, orderReq, gatewayName):
        """检查风险"""
        # 如果没有启动风控检查，则直接返回成功
        if not self.active:
            return True
        ...
        if workingOrderCount >= self.workingOrderLimit:
            msg = u'当前活动委托数量%s，超过限制%s' %(workingOrderCount, self.workingOrderLimit)
    ...
    #----------------------------------------------------------------------
    cpdef clearOrderFlowCount(self):
        """清空流控计数"""
        self.orderFlowCount = 0
        self.writeRiskLog(u'清空流控计数')

    #----------------------------------------------------------------------
    cpdef clearTradeCount(self):
        """清空成交数量计数"""
        self.tradeCount = 0
        self.writeRiskLog(u'清空总成交计数')

    #----------------------------------------------------------------------
    cpdef setOrderFlowLimit(self, n):
        """设置流控限制"""
        self.orderFlowLimit = n

    #----------------------------------------------------------------------
    cpdef setOrderFlowClear(self, n):
        """设置流控清空时间"""
        self.orderFlowClear = n

    #----------------------------------------------------------------------
    cpdef setOrderSizeLimit(self, n):
        """设置委托最大限制"""
        self.orderSizeLimit = n

    #----------------------------------------------------------------------
    cpdef setTradeLimit(self, n):
        """设置成交限制"""
        self.tradeLimit = n

    #----------------------------------------------------------------------
    cpdef setWorkingOrderLimit(self, n):
        """设置活动合约限制"""
        self.workingOrderLimit = n

    #----------------------------------------------------------------------
    cpdef setOrderCancelLimit(self, n):
        """设置单合约撤单次数上限"""
        self.orderCancelLimit = n

    #----------------------------------------------------------------------
    cpdef setMarginRatioLimit(self, n):
        """设置保证金比例限制"""
        self.marginRatioLimit = n/100   # n为百分数，需要除以100

    #----------------------------------------------------------------------
    cpdef switchEngineStatus(self):
        """开关风控引擎"""
