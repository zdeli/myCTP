...
class OIStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        if len(mysqlpositionInfo):
            for i in range(len(mysqlpositionInfo)):
                ## -------------------------------------------------
                if mysqlpositionInfo.at[i, 'direction'] == u'long':
                    tempDirection = u'sell'
                elif mysqlpositionInfo.at[i, 'direction'] == u'short':
                    tempDirection = u'cover'
                ## -------------------------------------------------
    ...
    def onTrade(self, trade):
        ...
        cdef char* tempKeyTrade = tempKey
        cdef char* tempKeyStart
        ## ---------------------------------------------------------------------
        ...
        ## 更新数量
        ## 更新交易日期
        if (self.stratTrade['vtOrderID'] in self.vtOrderIDListOpen and 
            tempKeyTrade in self.tradingOrdersOpen.keys()):
            # ------------------------------------------------------------------
            self.tradingOrdersOpen[tempKeyTrade]['volume'] -= self.stratTrade['volume']
            if self.tradingOrdersOpen[tempKeyTrade]['volume'] == 0:
                self.tradingOrdersOpen.pop(tempKeyTrade, None)
                self.tradedOrdersOpen[tempKeyTrade] = tempKeyTrade
            # ------------------------------------------------------------------
        elif (self.stratTrade['vtOrderID'] in self.vtOrderIDListClose and 
              tempKeyTrade in self.tradingOrdersClose.keys()):
            # ------------------------------------------------------------------
            self.tradingOrdersClose[tempKeyTrade]['volume'] -= self.stratTrade['volume']
            if self.tradingOrdersClose[tempKeyTrade]['volume'] == 0:
                self.tradingOrdersClose.pop(tempKeyTrade, None)
                self.tradedOrdersClose[tempKeyTrade] = tempKeyTrade
            # ------------------------------------------------------------------
        elif (self.stratTrade['vtOrderID'] in self.vtOrderIDListFailedInfo and
              tempKeyTrade in self.tradingOrdersFailedInfo.keys()):
            # ------------------------------------------------------------------
            self.tradingOrdersFailedInfo[tempKeyTrade]['volume'] -= self.stratTrade['volume']
            if self.tradingOrdersFailedInfo[tempKeyTrade]['volume'] == 0:
                self.tradingOrdersFailedInfo.pop(tempKeyTrade, None)
            # ------------------------------------------------------------------
            ## 需要更新一下 failedInfo
            self.stratTrade['TradingDay'] = self.ctaEngine.lastTradingDate
            self.processTradingOrdersFailedInfo(self.stratTrade)
        elif (self.stratTrade['vtOrderID'] in self.vtOrderIDListUpperLowerCum and 
              tempKeyTrade in self.tradingOrdersUpperLowerCum.keys()):
            # ------------------------------------------------------------------
            self.tradingOrdersUpperLowerCum[tempKeyTrade]['volume'] -= self.stratTrade['volume']
            # ------------------------------------------------------------------
            if self.tradingOrdersUpperLowerCum[tempKeyTrade]['volume'] == 0:
                self.tradingOrdersUpperLowerCum.pop(tempKeyTrade, None)
                self.tradedOrdersUpperLowerCum[tempKeyTrade] = tempKeyTrade
        elif (self.stratTrade['vtOrderID'] in self.vtOrderIDListWinner and 
              tempKeyTrade in self.tradingOrdersWinner.keys()):
            # ------------------------------------------------------------------
            self.tradingOrdersWinner[tempKeyTrade]['volume'] -= self.stratTrade['volume']
            # ------------------------------------------------------------------
            if self.tradingOrdersWinner[tempKeyTrade]['volume'] == 0:
                self.tradingOrdersWinner.pop(tempKeyTrade, None)
                self.tradedOrdersWinner[tempKeyTrade] = tempKeyTrade
            ...
            if self.tradingStart:
                ...
                if self.accountID not in self.WINNER_STRATEGY:
                    ...
                    if datetime.now().hour in [9,21] and datetime.now().minute < 2:
                        ...
                        ## ---------------------------------------------------------
                        if tempKeyStart in self.tradingOrdersUpperLowerCum.keys():
                            self.tradingOrdersUpperLowerCum[tempKeyStart]['volume'] += self.stratTrade['volume']
                        else:
                            ## -----------------------------------------------------
                            ## 生成 tradingOrdersUpperLowerCum
                            self.tradingOrdersUpperLowerCum[tempKeyStart] = {
                        ...
                        self.tradingOrdersUpperLower[tempKeyStart] = {
                                'vtSymbol'      : self.stratTrade['vtSymbol'],
                                'direction'     : tempDirection,
                                'volume'        : self.stratTrade['volume'],
                                'TradingDay'    : self.stratTrade['TradingDay'],
                        ...
                        self.tradingOrdersUpperLower[tempKeyStart]['vtOrderIDList'] = json.dumps(self.tradingOrdersUpperLower[tempKeyStart]['vtOrderIDList'])
                        tempRes = pd.DataFrame([[self.tradingOrdersUpperLower[tempKeyStart][k] for k in tempFields]], 
                                                columns = tempFields)
                    ...
                    ## 止盈平仓单
                    ## -------------------------------------------------------------
                    if tempKeyStart in self.tradingOrdersWinner.keys():
                        self.tradingOrdersWinner[tempKeyStart]['volume'] += self.stratTrade['volume']
                    else:
                        ## ---------------------------------------------------------
                        ## 生成 tradingOrdersWinner
                        self.tradingOrdersWinner[tempKeyStart] = {
                                'vtSymbol'      : self.stratTrade['vtSymbol'],
                                'direction'     : tempDirection,
                                'volume'        : self.stratTrade['volume'],
                                'TradingDay'    : self.stratTrade['TradingDay'],
        ...
        if trade.vtOrderID in self.vtOrderIDListOpen + self.vtOrderIDListClose:
